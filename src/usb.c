...
static int usb_console_pty_init(void) {
    ...
    /*
     * Do not keep the slave FD open in the emulator. On macOS, holding the
     * openpty() slave while also writing the master prevents external clients
     * (tio/screen) from reading the same slave node. Pending TX covers the
     * window before a client attaches.
     */
    close(slave);
...
/*
 * Finish DoTestWifi like firmware: param error + BE IPs, four C strings in
 * dataBuffer, MODE_LINEAR, reset data/param pointers so CP PrintStringFromDataBuffer
 * walks IP / netmask / gateway / DNS correctly.
 */
static void a2bus_complete_testwifi_ok(void) {
    /* parameterBuffer: err, then 4x IPv4 in network byte order (BE on wire) */
    ...
    mem_write8(USB_GUEST_PARAMETER_BUFFER + 4u, 2);   /* 192.168.4.2 */
    ...
    mem_write8(USB_GUEST_PARAMETER_BUFFER + 8u, 0);    /* 255.255.255.0 */
    ...
    mem_write8(USB_GUEST_PARAMETER_BUFFER + 12u, 1);   /* gw 192.168.4.1 */
    ...
    mem_write8(USB_GUEST_PARAMETER_BUFFER + 16u, 1);   /* dns 192.168.4.1 */
    ...
    mem_write8(USB_GUEST_REGISTERS + 1u, mem_read8(USB_GUEST_PARAMETER_BUFFER)); /* PARAMREG */
    mem_write8(USB_GUEST_REGISTERS + 2u, mem_read8(USB_GUEST_DATA_BUFFER));      /* DATAREG */
}
 ...
 *
 * Configured-SSID: full cyw43 gSPI join still WIP under a2bus. Complete DoTestWifi
 * with synthetic STA strings in dataBuffer (do not rely on guest ip4addr_ntoa).
 */
    ...
    if (mem_read8(USB_GUEST_WIFI_SSID) != 0u) {
        if (pc == USB_GUEST_DO_TEST_WIFI || pc == 0x20004548u) {
            fprintf(stderr,
                    ...
                    "(192.168.4.2/24; no host TAP/NAT on this path)\n");
            fflush(stderr);
        ...
        if (pc == USB_GUEST_TEST_WIFI) {
            /* Belt-and-suspenders if IPC already reached core0. */
        ...
        /* GetNetworkTime with SSID set — leave for later / real path. */
        return 0;
    }
...
void usb_console_guest_stdio_hook(void) {
    ...
    if (usb_mode && pc == USB_GUEST_PACKET_ASSERT_BHI1) {
        cpu.r[15] = 0x10003ff2u | 1u;
        return;
    }
    if (usb_mode && pc == USB_GUEST_PACKET_ASSERT_BHI2) {
        cpu.r[15] = 0x10003ff6u | 1u;
        return;
    }
    ...
    if (pc == USB_GUEST_U2_MONINIT_CALL) {
        cpu.r[15] = 0x100067fcu | 1u;
        return;
    }
    if (pc == USB_GUEST_U2_RESET_CALL) {
        cpu.r[15] = 0x10006816u | 1u;
        return;
    }
    ...
    if (pc == USB_GUEST_IS_APPLE_CONNECTED_CALL ||
        ...
        cpu.r[0] = 0;
        cpu.r[15] = (pc == USB_GUEST_IS_APPLE_CONNECTED_CALL ? 0x1000033cu
                                                              : 0x1000040cu) |
                     1u;
        return;
    }
    ...
    if (pc == USB_GUEST_CHECK_PICOW_CALL || pc == USB_GUEST_CHECK_PICOW_CALL2) {
        ...
        cpu.r[15] = (pc == USB_GUEST_CHECK_PICOW_CALL ? 0x1000039cu : 0x100003c2u) |
                     1u;
        return;
    }
    ...
    if (usb_mode &&
        ...
        cpu.r[15] = (pc == USB_GUEST_INIT_PICOLED_CALL1 ? 0x100003ceu
                                                         : 0x1000048eu) |
                     1u;
        return;
    }
    ...
    if (usb_mode && pc == USB_GUEST_USB_WAIT_LOOP) {
        cpu.r[15] = 0x10000414u | 1u;
        return;
    }
    if (usb_mode && pc == USB_GUEST_USB_CONN_LOOP) {
        cpu.r[15] = 0x10000490u | 1u;
        return;
    }
    if (pc == USB_GUEST_CLOCK_GET_HZ) {
        cpu.r[0] = 150000000u;
    ...
    if (pc == USB_GUEST_HW_CLAIM_CLEAR_FAIL) {
        cpu.r[15] = 0x1000af84u | 1u; /* pop return — clear on unclaimed bit is OK in emu */
        return;
    }
    if (pc == USB_GUEST_TS_READ_JEDECID) {
        cpu.r[0] = USB_GUEST_WINBOND_JEDEC24;
