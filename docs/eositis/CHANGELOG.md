...
| Change | Reason |
|--------|--------|
| Docs: host-complete DoTFTPStatus from tftp_state | Native path HardFaulted core1 (PC="RunT…") after DoTFTPRun |
...
## 2026-08-03 — `946c705` — TestWifi IP strings via strcpy + real DHCP
...
| Change | Reason |
|--------|--------|
| `-wifi` enables `_vfprintf_r` / mbtowc guest hooks; remap corrupt `0x30` flash PCs | Dual-core locale smash → HardFault → option 7 Not Found |
...
## 2026-08-01 — `e2a64ef` — TestWifi host DNS docs
