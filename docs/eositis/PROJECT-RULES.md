...
- macOS CYW43 internet: `-wifi -tap` uses utun; see `docs/eositis/MACOS-WIFI.md` and `scripts/macos-cyw43-pf-nat.sh`. MegaFlash MAME launcher integrates admin dialog + pf + `-tap`; overlay stubs boot `cyw43_arch_init` then arms real JOIN on TestWifi/NTP.
